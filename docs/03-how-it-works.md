...
Presentation-state semantics are intentionally strict: **Off** covers healthy batteries with no useful headroom, including full, maximum-power, and taper-saturated conditions; **Requested** is reserved for unresolved usable-battery telemetry; **Waiting** means HBC requests charging and known headroom exists, but measured charging is not yet confirmed or no currently usable PV increase exists; **Active** requires confirmed charging operation. This prevents a normal no-headroom condition from being reported as an availability fault.
