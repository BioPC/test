...
### Diagnostic sensor migration

The Market/export diagnostic sensor is now `sensor.hpvc_diag_market_export_price`. Existing installations using `sensor.hpvc_diag_market_price` must update custom dashboards, automations, and external references. After verifying the new sensor, the old registry entity can be removed from Home Assistant.
